...
export const NexusProvider: React.FC<NexusProviderProps> = ({
  ...
  function walletHasGetProvider(
    w: unknown
  ): w is { getProvider: () => Promise<unknown> } {
    return !!w && typeof (w as any).getProvider === "function";
  }

  const initializeSDK = useCallback(async () => {
    if (isConnected && !nexusSdk && account) {
      try {
        ...
        const provider = walletHasGetProvider(wallet)
          ? ((await wallet.getProvider()) as EthereumProvider)
  ...
  }, [isConnected, nexusSdk, wallet, account]);
